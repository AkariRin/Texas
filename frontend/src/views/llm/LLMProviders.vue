<template>
  <v-container fluid>
    <v-card flat>
      ...
      <v-card-text v-else-if="store.providers.length === 0" class="text-center py-8 text-medium-emphasis">
        暂无提供商，点击右上角添加
      </v-card-text>

      <v-row v-else class="pa-4" dense>
        <v-col v-for="provider in store.providers" :key="provider.id" cols="12" sm="6" md="4" lg="3">
