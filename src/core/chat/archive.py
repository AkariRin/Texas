"""聊天记录归档服务与 Celery 任务 —— 编排冷数据归档流程（发现分区 → 导出 → 上传 S3 → 清理）。"""
...
from src.core.chat.exporter import (
    _PARTITION_NAME_RE,
    ChatArchiveSettings,
    ParquetExporter,
)
...
def _build_services() -> tuple[Any, Any]:
    ...
    from src.core.chat.exporter import ChatArchiveSettings
    from src.core.chat.main import ChatDatabaseSettings
    from src.core.chat.s3 import S3Settings
