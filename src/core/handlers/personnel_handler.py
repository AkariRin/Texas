...
class PersonnelEventHandler:
    ...
    通过 ``ctx.get_service(PersonnelEventService)`` 获取事件服务实例，
    ...
    async def on_friend_add(self, ctx: Context) -> bool:
        ...
        if not ctx.has_service(PersonnelEventService):
            return False
        ...
        try:
            ...
            await event_service.on_friend_add(user_id)
    ...
    async def on_group_increase(self, ctx: Context) -> bool:
        ...
        try:
            ...
            await event_service.on_group_increase(group_id, user_id, self_id=bot_qq)
        except Exception as exc:
            logger.error(
                "处理群成员增加事件失败",
                group_id=group_id,
    ...
    async def on_group_decrease(self, ctx: Context) -> bool:
        ...
        try:
            event_service = ctx.get_service(PersonnelEventService)
            await event_service.on_group_decrease(group_id, user_id, sub_type)
        except Exception as exc:
            logger.error(
                "处理群成员减少事件失败",
                group_id=group_id,
    ...
    async def on_group_admin(self, ctx: Context) -> bool:
        ...
        try:
            event_service = ctx.get_service(PersonnelEventService)
            await event_service.on_group_admin_change(group_id, user_id, sub_type)
        except Exception as exc:
            logger.error(
                "处理群管理员变动事件失败",
                group_id=group_id,
    ...
    async def on_group_card(self, ctx: Context) -> bool:
        ...
        try:
            event_service = ctx.get_service(PersonnelEventService)
            await event_service.on_group_card_change(group_id, user_id, card_new)
        except Exception as exc:
            logger.error(
                "处理群名片变更事件失败",
                group_id=group_id,
