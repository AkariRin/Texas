...
class GoCQHTTPAPIMixin:
    ...
    async def set_qq_profile(
        self, nickname: str, company: str = "", email: str = "", college: str = "",
        personal_note: str = "",
    ) -> APIResponse:
        return await self._call("set_qq_profile", {
            "nickname": nickname, "company": company, "email": email,
            "college": college, "personal_note": personal_note,
        })
    ...
    async def upload_group_file(
        self, group_id: int, file: str, name: str, folder: str = "",
    ) -> APIResponse:
    ...
    async def get_group_file_url(self, group_id: int, file_id: str, busid: int) -> APIResponse:
        return await self._call("get_group_file_url", {
            "group_id": group_id, "file_id": file_id, "busid": busid,
        })

    async def download_file(self, url: str, thread_count: int = 1, headers: str = "") -> APIResponse:
        return await self._call("download_file", {
            "url": url, "thread_count": thread_count, "headers": headers,
        })
